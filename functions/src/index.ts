...
type TxnSnap = admin.firestore.DocumentSnapshot<admin.firestore.DocumentData>;

const walletScopeCache = new Map<string, string | null>();

const getPeriodKey = (date: admin.firestore.Timestamp): string => {
  const d = date.toDate();
...
const getWalletScope = async (walletId: string): Promise<string | null> => {
  ...
  const scope = (snap.get('scope') as string | undefined) ?? null;
...
const fetchMemberBudgets = async (
  ...
  const docs = query.docs
    ...
    .map((doc) => ({
      ...(doc.data() as BudgetDoc),
      budgetId: doc.id,
    }));
  console.log(
    '[BUDGET_CF] query member budgets household=%s period=%s uid=%s count=%d (filtered from %d)',
    householdId,
    periodKey,
    memberUid,
    docs.length,
...
const fetchFamilyBudgets = async (householdId: string, periodKey: string): Promise<BudgetDoc[]> => {
  // Query with minimal fields to avoid composite index dependency
  ...
  console.log(
    '[BUDGET_CF] query family budgets household=%s period=%s count=%d (filtered from %d)',
    householdId,
    periodKey,
    docs.length,
    query.docs.length
...
const buildTxnContext = async (snap: TxnSnap): Promise<TxnContext> => {
  const data = snap.data();
  const walletId = (data?.wallet_id as string | undefined) ?? null;
  const walletScope = walletId ? await getWalletScope(walletId) : null;
  const actorUserId =
    (data?.actor_user_id as string | undefined) ?? (data?.user_id as string | undefined) ?? null;
...
const shouldNotifyLevel = (spent: number, limit: number, lastLevel: number): number => {
  if (limit <= 0) return 0;
  const percent = (spent / limit) * 100;
  let level = 0;
  for (const t of thresholds) {
    if (percent >= t) level = t;
  }
  return level > lastLevel ? level : 0;
};

const resolveHeadUserId = async (householdId: string, fallback?: string): Promise<string | null> => {
  ...
  return (snap.docs[0].data().user_id as string) ?? null;
};

const createNotifications = async (userIds: string[], budgetId: string, level: number) => {
  if (userIds.length === 0 || level === 0) return;
...
const applyUsageDelta = async (
  ...
  delta: number,
  actorUserId?: string,
  txnData?: admin.firestore.DocumentData,
  walletScope?: string | null
) => {
  if (delta === 0) return;
  if (txnData && !matchBudget(budget, txnData, actorUserId)) return;
  ...
  await db.runTransaction(async (txn) => {
    ...
    console.log(
      '[BUDGET_CF_USAGE_WRITE] budget=%s usage_doc=%s period=%s delta=%s spent_before=%s spent_after=%s actor=%s wallet_scope=%s',
      budget.budgetId,
      usageRef.id,
      periodKey,
      delta,
...
const matchBudget = (
  budget: BudgetDoc,
  txn: admin.firestore.DocumentData,
  actorUserId?: string
): boolean => {
  if (budget.archived) return false;
  if (budget.wallet_scope_filter && budget.wallet_scope_filter !== 'household_shared') {
    return false;
  }
  if (budget.budget_type === 'member' && budget.member_user_id) {
    if (actorUserId !== budget.member_user_id) return false;
  }
  return true;
};

const applyContextDelta = async (
  ctx: TxnContext,
  delta: number,
  txnData: admin.firestore.DocumentData
) => {
  if (!ctx.eligible || !ctx.periodKey || !ctx.householdId || delta === 0) {
    console.log(
      '[BUDGET_CF] skip applyDelta eligible=%s period=%s household=%s delta=%s',
      ctx.eligible,
      ctx.periodKey,
      ctx.householdId,
      delta
  ...
  console.log(
    '[BUDGET_CF] matchedBudgets member=%d family=%d period=%s delta=%s actor=%s wallet_scope=%s household=%s',
    memberBudgets.length,
    familyBudgets.length,
    ctx.periodKey,
    delta,
    ctx.actorUserId ?? 'null',
    ctx.walletScope ?? 'null',
    ctx.householdId
  );
  let appliedCount = 0;
  for (const budget of [...memberBudgets, ...familyBudgets]) {
    if (!matchBudget(budget, txnData, ctx.actorUserId)) {
      console.log(
        '[BUDGET_CF] skip budget=%s type=%s member=%s (match failed)',
        budget.budgetId,
        budget.budget_type,
        budget.member_user_id ?? 'null'
      );
      continue;
    }
...
const handleCreate = async (snap: TxnSnap) => {
  const ctx = await buildTxnContext(snap);
  const data = snap.data() ?? {};
  console.log(
    '[BUDGET_CF] event=create txn=%s type=%s amount=%s period=%s wallet=%s scope=%s actor=%s household=%s eligible=%s',
    snap.id,
    data.type,
    data.amount,
    ctx.periodKey,
...
const handleDelete = async (snap: TxnSnap) => {
  const ctx = await buildTxnContext(snap);
  const data = snap.data() ?? {};
  console.log(
    '[BUDGET_CF] event=delete txn=%s type=%s amount=%s period=%s wallet=%s scope=%s actor=%s household=%s eligible=%s',
    snap.id,
    data.type,
    data.amount,
    ctx.periodKey,
...
const handleUpdate = async (before: TxnSnap, after: TxnSnap) => {
  ...
  console.log(
    '[BUDGET_CF] event=update txn=%s amountBefore=%s amountAfter=%s actorBefore=%s actorAfter=%s householdBefore=%s householdAfter=%s eligibleBefore=%s eligibleAfter=%s periodBefore=%s periodAfter=%s scopeBefore=%s scopeAfter=%s',
    after.id,
    beforeCtx.amount,
    afterCtx.amount,
    beforeCtx.actorUserId,
    afterCtx.actorUserId,
    beforeCtx.householdId,
    afterCtx.householdId,
    beforeCtx.eligible,
    afterCtx.eligible,
    beforeCtx.periodKey,
    afterCtx.periodKey,
    beforeCtx.walletScope,
    afterCtx.walletScope
  );
